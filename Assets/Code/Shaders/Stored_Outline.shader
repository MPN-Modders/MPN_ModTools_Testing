...
/////////////////////////////////////////
	...
	Tags {"LightMode" = "ForwardBase""IgnoreProjector" = "True"}//"Queue"="Transparent+60" "IgnoreProjector"="True"} "RenderType"="Transparent+60"}
		...
		Tags { "LightMode" = "ForwardBase"  "RenderType"="TransparentCutout""IgnoreProjector"="True"}
			
			//Blend SrcAlpha One
			ZTest Always	// NOTE [2/8/19] 
			Cull Front
			Lighting Off
			//Zwrite Off	// NOTE [2/8/19] This is cool - if we leave this ON, then when behind a wall/floor, the outline overwrites whatever it was printed over (thanks to ZTest Always). But with Zwrite ON, it changes the depth buffer of
